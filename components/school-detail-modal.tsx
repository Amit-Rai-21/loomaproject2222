...
import { MapPin, Monitor, Mail, Phone, User, Terminal, QrCode, History, Building, Globe, X } from "lucide-react"
...
export function SchoolDetailModal({ school, isOpen, onClose }: SchoolDetailModalProps) {
  ...
  const openTerminal = () => {
    ...
    setTerminalLines([
      ...
      `Last login: ${format(new Date(school.lastSeen), "PPp")}`,
      "",
    ])
  }
  ...
  const statusConfig = {
    online: { className: "bg-green-500/10 text-green-600 border-green-500/20", dot: "bg-green-500" },
    offline: { className: "bg-red-500/10 text-red-600 border-red-500/20", dot: "bg-red-500" },
    maintenance: { className: "bg-yellow-500/10 text-yellow-600 border-yellow-500/20", dot: "bg-yellow-500" },
  }
          ...
          <div className="absolute bottom-4 left-6 right-6">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <DialogDescription className="text-sm text-white/90 mt-1 flex items-center gap-1">
                  <MapPin className="h-4 w-4" />
                  {school.palika}, {school.district}, {school.province}
                </DialogDescription>
              </div>
              <Badge variant="outline" className={`${statusConfig[school.status].className} bg-white/90`}>
                <span className={`w-2 h-2 rounded-full ${statusConfig[school.status].dot} mr-1.5`} />
                {school.status}
              </Badge>
            </div>
          </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <Badge variant="outline" className={statusConfig[school.status].className + " text-xs"}>
                        {school.status}
                      </Badge>
                    </div>
                    <div className="flex justify-between">
                ...
                <CardContent>
                  {school.qrScans.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">
                      No QR scans recorded for this school.
                    </p>
                  ) : (
            ...
            <TabsContent value="access-logs" className="mt-0">
              <Card>
                ...
                <CardContent>
                  {school.accessLogs.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">
                      No access logs recorded for this school.
                    </p>
                  ) : (
                    <div className="space-y-3">
                ...
                <CardContent>
                  {school.status === "online" ? (
                    terminalOpen ? (
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Type <code className="bg-muted px-1 rounded">help</code> for available commands, <code className="bg-muted px-1 rounded">exit</code> to close
                        </p>
                      </div>
                    ) : (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Remote access is unavailable. This Looma is currently <strong>{school.status}</strong>.
                      </p>
                    </div>
                  )}
                </CardContent>
